			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	params.mode = 0; /* 0 infra 1 IBSS 2 AP */
	for (i = 0; i < NUM_WEP_KEYS; i++) {
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->driver->send_eapol == NULL) {
		wpa_printf(MSG_ERROR, "error: driver not support send_eapol\n");
		return -1;
	} 
...
int wpa_supplicant_init_iface(struct wpa_supplicant *wpa_s)
